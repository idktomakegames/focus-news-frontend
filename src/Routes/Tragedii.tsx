...
export default function Tragedii() {
    ...
    useEffect(() => {
      ...
      const res = await fetch(`https://focus-news-backend-production.up.railway.app/get-articles/category/tragedii/${currentPage}`)
    ...
    return (
      <>
      <Navbar/>
      <h1 className='text-center text-4xl md:text-5xl font-semibold py-10'>Categorie: Tragedii</h1>
      <div className='flex justify-center'>
          <div className="grid gap-5" id="articlesDisplay">
            {articles?.length > 0 && (
                <Link to={`/article/${articles[0]?._id}`} className='col-span-full'>
                <fieldset className="border-2 border-gray-400 rounded-lg flex flex-col items-center">
