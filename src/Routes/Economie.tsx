...
export default function Economie() {
    ...
    useEffect(() => {
      ...
      const res = await fetch(`https://focus-news-backend-production.up.railway.app/get-articles/category/economie/${currentPage}`)
    ...
    return (
      <>
      <Navbar/>
      <h1 className='text-center text-4xl md:text-5xl font-semibold py-10'>Categorie: Economie</h1>
      <div className='flex justify-center'>
          <div className="grid gap-5" id="articlesDisplay">
            ...
            {articles?.map((article, index) => <Link to={`/article/${article._id}`} key={index}>
              <fieldset className="border-2 border-gray-400 h-80 rounded-lg flex flex-col items-center w-auto">
